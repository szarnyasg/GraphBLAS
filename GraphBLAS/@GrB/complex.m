...
%COMPLEX cast a GraphBLAS matrix to MATLAB sparse double _Complex matrix.
...
%
% To typecast the matrix G to a GraphBLAS sparse double _Complex matrix
% instead, use C = GrB (G, 'complex') or C = GrB (G, 'double _Complex').
...
if (nargin == 1)
    ...
    % Convert A to a double _Complex matrix C.
    C = gbsparse (A.opaque, 'double _Complex') ;
    ...
    % convert C to a MATLAB double _Complex matrix
    C = gbsparse (C.opaque, 'double _Complex') ;
